import SwiftUI
import Supabase
import Observation

@Observable
@MainActor
final class LifeLobbyViewModel {
    var games: [LifeGameRow] = []
    var usernames: [UUID: String] = [:]
    var loading: Bool = true
    var creating: Bool = false
    var createdGame: LifeGameRow? = nil
    var errorMessage: String? = nil

    private let client = SupabaseService.client
    private var subscriptionTask: Task<Void, Never>?

    func load() async {
        loading = true
        defer { loading = false }
        do {
            let rows = try await LifeService.fetchOpenGames()
            self.games = rows
            await fetchUsernames(for: Array(Set(rows.map { $0.host })))
        } catch {
            errorMessage = error.localizedDescription
        }
    }

    func subscribe() {
        subscriptionTask?.cancel()
        subscriptionTask = Task { [weak self] in
            guard let self else { return }
            await self.client.realtimeV2.connect()
            let channel = self.client.realtimeV2.channel("life:lobby")
            let inserts = channel.postgresChange(
                InsertAction.self, schema: "public", table: "life_games"
            )
            let updates = channel.postgresChange(
                UpdateAction.self, schema: "public", table: "life_games"
            )
            do { try await channel.subscribeWithError() } catch { return }
            await withTaskGroup(of: Void.self) { group in
                group.addTask { [weak self] in
                    for await action in inserts {
                        if let row = try? action.decodeRecord(as: LifeGameRow.self, decoder: JSONDecoder()) {
                            await self?.handleInsert(row)
                        }
                    }
                }
                group.addTask { [weak self] in
                    for await action in updates {
                        if let row = try? action.decodeRecord(as: LifeGameRow.self, decoder: JSONDecoder()) {
                            await self?.handleUpdate(row)
                        }
                    }
                }
            }
        }
    }

    func unsubscribe() {
        subscriptionTask?.cancel()
        subscriptionTask = nil
    }

    func createGame(userId: String) async {
        guard !creating else { return }
        creating = true
        defer { creating = false }
        do {
            let row = try await LifeService.createGame(userId: userId)
            self.createdGame = row
        } catch {
            errorMessage = error.localizedDescription
        }
    }

    private func handleInsert(_ row: LifeGameRow) async {
        let cutoff = Date().addingTimeInterval(-2 * 3600)
        let ts = Self.parseDate(row.created_at) ?? Date()
        if row.statusEnum == .waiting && ts >= cutoff {
            if !games.contains(where: { $0.id == row.id }) {
                games.insert(row, at: 0)
                await fetchUsernames(for: [row.host])
            }
        }
    }

    private func handleUpdate(_ row: LifeGameRow) async {
        if row.statusEnum == .waiting {
            if let i = games.firstIndex(where: { $0.id == row.id }) {
                // Keep the seat count fresh as players join the waiting room.
                games[i] = row
            } else {
                games.insert(row, at: 0)
                await fetchUsernames(for: [row.host])
            }
        } else {
            games.removeAll { $0.id == row.id }
        }
    }

    private func fetchUsernames(for ids: [UUID]) async {
        let missing = ids.filter { usernames[$0] == nil }
        for id in missing {
            if let name = try? await LifeService.fetchUsername(userId: id) {
                usernames[id] = name
            }
        }
    }

    nonisolated(unsafe) private static let isoFormatter: ISO8601DateFormatter = {
        let f = ISO8601DateFormatter()
        f.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
        return f
    }()
    private static func parseDate(_ s: String) -> Date? {
        if let d = isoFormatter.date(from: s) { return d }
        let alt = ISO8601DateFormatter()
        alt.formatOptions = [.withInternetDateTime]
        return alt.date(from: s)
    }
}
