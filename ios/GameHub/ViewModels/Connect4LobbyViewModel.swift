...
@Observable
@MainActor
final class Connect4LobbyViewModel {
    var games: [Connect4GameRow] = []
    var usernames: [UUID: String] = [:]
    var loading: Bool = true
    var creating: Bool = false
    var createdGame: Connect4GameRow? = nil
    ...
    func load() async {
        ...
        defer { loading = false }
        do {
            let rows = try await Connect4Service.fetchOpenGames()
            self.games = rows
    ...
    func subscribe() {
        ...
        subscriptionTask = Task { [weak self] in
            ...
            let channel = self.client.realtimeV2.channel("connect4:lobby")
            let inserts = channel.postgresChange(
                InsertAction.self, schema: "public", table: "connect4_games"
            )
    ...
    func createGame(userId: String) async {
        ...
        do {
            let row = try await Connect4Service.createGame(userId: userId)
    ...
    private func handleInsert(_ row: Connect4GameRow) async {
    ...
    private func handleUpdate(_ row: Connect4GameRow) async {
    ...
    private func fetchUsernames(for ids: [UUID]) async {
        ...
        for id in missing {
            if let name = try? await Connect4Service.fetchUsername(userId: id) {
                usernames[id] = name
            }
        }
    }
