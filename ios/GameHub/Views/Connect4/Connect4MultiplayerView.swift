...
struct Connect4MultiplayerView: View {
    @State private var viewModel: Connect4MultiplayerViewModel
    @Environment(\.dismiss) private var dismiss

    init(initialGame: Connect4GameRow) {
        let uid = AuthService.shared.currentUserId ?? ""
        _viewModel = State(initialValue: Connect4MultiplayerViewModel(initialGame: initialGame, currentUserId: uid))
    }
    ...
    var body: some View {
        ...
        .navigationDestination(item: $viewModel.rematchGame) { row in
            Connect4MultiplayerView(initialGame: row)
        }
    ...
    private var gameBody: some View {
        VStack(spacing: 14) {
            ...
            Connect4BoardView(
                ...
                showArrows: viewModel.isMyTurn && !viewModel.game.isGameOver,
    ...
    private var gameOverCard: some View {
        ...
        let title: String = {
            ...
            return status == .p1_wins ? "Red wins!" : "Yellow wins!"
        }()
    ...
    private var playerIndicator: some View {
        if let me = viewModel.myPlayer, viewModel.game.statusEnum != .waiting {
            HStack(spacing: 6) {
                ...
                Text(me == .one ? "🔴 Red" : "🟡 Yellow")
                    .foregroundStyle(me == .one ? Color.hubRose : Color.hubAmber)
                    .fontWeight(.black)
