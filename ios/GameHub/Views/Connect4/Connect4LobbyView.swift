...
struct Connect4LobbyView: View {
    @State private var viewModel = Connect4LobbyViewModel()
    ...
    @ViewBuilder
    private func lobbyRow(_ game: Connect4GameRow) -> some View {
        let isMine = game.player_1.uuidString.lowercased() == (AuthService.shared.currentUserId ?? "").lowercased()
        let name = isMine ? "⭐ You" : "🔴 \(viewModel.usernames[game.player_1] ?? "Someone")"
        NavigationLink {
            Connect4MultiplayerView(initialGame: game)
        } label: {
            HStack {
