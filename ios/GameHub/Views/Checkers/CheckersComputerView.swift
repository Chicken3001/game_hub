...
struct CheckersComputerView: View {
    @State private var viewModel = CheckersComputerViewModel()
    ...
    var body: some View {
        ...
        .navigationTitle("Checkers vs Computer")
    ...
    private var setupArea: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 10) {
                ForEach(CheckersDifficulty.allCases, id: \.self) { diff in
                    difficultyRow(diff)
                }
            }
            ...
            HStack {
                ...
                Spacer()
                Toggle("", isOn: $viewModel.forcedCapture)
                    .labelsHidden()
    ...
    private func difficultyRow(_ diff: CheckersDifficulty) -> some View {
    ...
    private var playerIndicator: some View {
        HStack(spacing: 6) {
            ...
            Text(viewModel.humanPlayer == .one ? "🔴 Red" : "⚫ Black")
                .foregroundStyle(viewModel.humanPlayer == .one ? Color.hubRose : Color.hubInk)
                .fontWeight(.black)
